    <div>
            ...
            <div class="d-block mb-4 mb-md-0">
                <nav aria-label="breadcrumb" class="d-none d-md-inline-block">
                    <ol class="breadcrumb breadcrumb-dark breadcrumb-transparent">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Manajemen Program Studi</li>
                    </ol>
                </nav>
                <h2 class="h4">Manajemen Program Studi</h2>
                <p class="mb-0"></p>
            </div>
            ...
            <table class="table table-hover">
                <thead>
                    <tr>
                        ...
                        <th class="border-gray-200">Fakultas</th>
                        <th class="border-gray-200">Program Studi</th>
                        <th class="border-gray-200">Kaprodi</th>
                    ...
                    <tr>
                        <td>
                            <a href="#" class="fw-bold">
                                {{ ($loop->index) + (($prodi->currentPage() - 1) * $prodi->perPage()) + 1 }}
                            </a>
                        </td>
                        <td>
                            <span class="fw-normal">{{ $prodiItem->fakultas->nama }}</span>
                        </td>
                        <td>
                            <span class="fw-normal">{{ $prodiItem->nama }}</span>
                        </td>
                        <td>
                            <span class="fw-normal">{{ $prodiItem->user->name }}</span>
                        </td>
                        <td><span class="fw-normal">{{ $prodiItem->slug }}</span></td>
                        <td class="text-end">
                            <a href="#" class="btn btn-info btn-sm btn-rounded" wire:click.prevent="edit({{ $prodiItem->id }})" data-bs-toggle="modal" data-bs-target="#modaEdit">Edit</a>
                            <a href="#" wire:click.prevent="hapus({{ $prodiItem->id }})" class="btn btn-danger btn-sm btn-rounded">Delete</a>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
            <div class="mt-2">{{ $prodi->links() }}</div>
        </div>
        <!-- Modal Content -->
        ...
        <div wire:ignore.self class="modal fade" id="modaEdit" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content ">
                    <div class="modal-header">
                        <h2 class="h6 modal-title">Update Fakultas</h2>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <form>
                        @csrf
